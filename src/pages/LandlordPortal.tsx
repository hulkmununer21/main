...
const LandlordPortal = () => {
  ...
  // Find an admin contact on load (to allow "Start Chat")
  useEffect(() => {
    const findAdmin = async () => {
        // Just pick the first staff member for now as "Support"
          ...
          <main className="flex-1 p-8 overflow-auto bg-muted/30 pb-24">
            ...
            {currentTab === "overview" && (
              <div className="space-y-6">
                ...
                {/* ✅ RECENT ACTIVITY (Updated to show Notifications) */}
                <Card>
                    <CardHeader>
                    ...
                    <CardContent>
                        <div className="space-y-4">
                            {notifications.length === 0 ? (
                                ...
                                notifications.slice(0, 5).map(n => ( // Fetch only 5
                                    <div key={n.id} className="flex justify-between items-start border-b pb-3 last:border-0">
                                        <div>
                                            <div className="flex items-center gap-2">
                                                <p className={`font-medium ${!n.is_read ? 'text-primary' : ''}`}>{n.subject}</p>
                                                {!n.is_read && <Badge className="h-5 px-1.5 text-[10px] bg-blue-500">New</Badge>}
                                            </div>
                                            <p className="text-xs text-muted-foreground line-clamp-1">{n.message_body}</p>
                                        </div>
